...
var makeEnsemblecomposite = function(annualSRcollection, reducer){
  ...
  var composite = standard.map(function(img){
    ...
    return reducedImg.multiply(1000).rename('ENC').set('system:time_start', img.get('system:time_start'));            
  });  
...
exports.makeEnsemblecomposite = makeEnsemblecomposite;
...
var standardizeIndex = function(collection, index){
  ...
  zCollection = zCollection.map(function(img){
    return img.multiply(1000).rename(index+'z').set('system:time_start', img.get('system:time_start'));  
  });
...
var getFittedData = function(lt, startYear, endYear, index, ftv){
  var bandNames = getYearBandNames(startYear, endYear, index);
...
var getYearBandNames = function(startYear, endYear, ftv){
  var years = [];                                                           // make an empty array to hold year band names
  for (var i = startYear; i <= endYear; ++i) years.push(ftv+'_'+i.toString()); // fill the array with years from the startYear to the endYear and convert them to string
  return years;
};
